...
import { Paper, Text, Group, Button, SimpleGrid, Tooltip, Card } from "@mantine/core";
import { IconChevronLeft, IconChevronRight, IconUserCheck, IconUserOff, IconClock } from '@tabler/icons-react';
...
export default function Attendes() {
  ...
  // setLoading(true); // removed unused loading state
      try {
        ...
        notifyError("Error: " + ((err instanceof Error ? err.message : "Failed to load")));
      } finally {
  ...
  const matrix = getMonthMatrix(year, month);
    ...
    return (
      <Paper shadow="xs" p="xl" withBorder style={{ minHeight: '100vh'}}>
        <SimpleGrid cols={3} spacing={24} mb={24}>
          <Card shadow="sm" padding="lg" radius="md" withBorder>
            <Group>
              <IconUserCheck color="#38d996" size={28} />
              <div>
                <Text size="md" fw={600} c="dimmed">Present Days</Text>
                <Text size="xl" fw={700}>{presentCount}</Text>
              </div>
            </Group>
          </Card>
          <Card shadow="sm" padding="lg" radius="md" withBorder>
            <Group>
              ...
              <div>
                <Text size="md" fw={600} c="dimmed">Absent Days</Text>
                <Text size="xl" fw={700}>{absentCount}</Text>
              </div>
            </Group>
          </Card>
          <Card shadow="sm" padding="lg" radius="md" withBorder>
            <Group>
              ...
              <div>
                <Text size="md" fw={600} c="dimmed">Leave Days</Text>
                <Text size="xl" fw={700}>{leaveCount}</Text>
              </div>
            </Group>
          </Card>
        </SimpleGrid>
        <Group justify="space-between" mb="lg">
          <Text fw={900} size="2rem" style={{ color: '#228be6' }}>Attendance Calendar</Text>
          <Group>
            <Button
              variant="light"
              color="blue"
        ...
        <SimpleGrid cols={7} spacing={8} style={{ flex: 1 }}>
          {matrix.flat().map((day, idx) => {
            ...
            const attendanceDay = day ? records.find(a => {
